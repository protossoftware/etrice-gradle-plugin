...
To debug the generator process, pass the `--debug-jvm` command line option to a generate task, e.g. `./gradlew generateRoom --debug-jvm`.
The process is then started suspended and listening on port 5005.
